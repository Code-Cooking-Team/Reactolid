...
export class Geometry extends EventDispatcher {
    ...
    merge(geometry, matrix, materialIndexOffset = 0) {
        if (!(geometry && geometry.isGeometry)) {
            console.error(
                'Geometry.merge(): geometry not an instance of Geometry.',
                geometry,
            )
            return
        }
    ...
    computeTangents() {
        console.error('Geometry: .computeTangents() has been removed.')
    }
    ...
    applyMatrix(matrix) {
        console.warn('Geometry: .applyMatrix() has been renamed to .applyMatrix4().')
